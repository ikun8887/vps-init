...
- v0.3：固定版本安装器下载带 SHA256 校验的单文件，安装 `vps-init` 快捷入口；菜单直接选模块，操作后返回菜单。支持重装、完整恢复卸载和保留配置卸载。
- v0.3：普通 SSH 变更保留原入口、本机验证后直接完成；严格关闭旧入口和身份加固继续要求新连接验证。旧版待确认记录先恢复再重试。详细命令输出写入有上限的日志，摘要直接同步输出到原会话。
